		...
		<?php if ( 1 < count( $available_methods ) ) : ?>
			<ul id="shipping_method">
				<?php foreach ( $available_methods as $method ) : ?>
					<li>
						<?php
							// Modified by Arlind (wrapped with label - class alternate-radio)
							printf( '<label for="shipping_method_%1$d_%2$s" class="alternate-radio"><input type="radio" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d_%2$s" value="%3$s" class="shipping_method" %4$s />
								%5$s</label>',
